...
* `$nextTick()` 是一个异步的方法在 dom异步队列执行后执行，常用在 vue 的更新后获取最新的dom数据。不直接使用 `setTimeout()` 代替 `nextTick()` 是因为 `setTimeout` 会开启一个宏任务，宏任务多性能也会差，`nextTick()` 会将所有需要更新的方法放入到队列中最后批量处理，比一个个的 `setTimeout()` 处理效率高很多，vue2.x 中为了兼容做了很多的降级处理`Promise.then, MutationObserver，setImmediate，setTimeout`。
``` js
var callbacks = [];
var pending = false;
...
### Vue.mixin() 实现原理
> 混入，将可以复用的功能混入组件内，同名钩子的函数会被合并入一个数组，都会被调用，混入的钩子先入的队列会先比组件中的钩子先执行，上面使用的是发布订阅模式
* mixin 是类的方法不是实例方法
``` js
