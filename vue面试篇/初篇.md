...
* vue 通过将 `render` 函数返回的虚拟 dom(vNode) 传递给 `vm._update(vm._render())` 更新渲染成真实 dom 替换老节点，上面节点的变化更新使用到了观察者模式，通过观察属性的变化更新视图。
``` js
...
* `Watcher` 观察者，根据 vm 属性的变化，调用 `vm._update()` 方法将虚拟 dom 更新。
...
* 取值时，每一个属性都添加了`dep`属性，用来存放渲染的 `watcher` 
...
### vue 异步批量更新原理 和 $nextTick()
[vue 异步更新原理](https://zhuanlan.zhihu.com/p/186281394)
[Vue.nextTick 的原理和用途](https://segmentfault.com/a/1190000012861862)
* vue 中的更新是异步的
* 多次修改的数据只保留最后一次放入到队列中，利用js异步的特性一次批量执行(防抖)
* `$nextTick()` 是一个异步的方法在 dom异步队列执行后执行，常用在 vue 的更新后获取最新的dom数据。不直接使用 `setTimeout()` 代替 `nextTick()` 是因为`setTimeout`会开启一个宏任务，如果宏任务多性能也会差，`nextTick()` 会将所有需要更新的方法放入到队列中最后批量处理，比一个个的`setTimeout()` 处理效率高很多，vue2.x 中为了兼容做了很多的降级处理`Promise, MutationsObserver`。
``` js
var callbacks = [];
var pending = false;
