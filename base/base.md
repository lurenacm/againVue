...
* `v-for` 循环绑定数据有两种写法 `v-for="(value, key, index) of list"/ v-for="(value, key, index) in list"`，`value`就是属性值不是属性名，每一个循环出来的节点都应该配有一个 `key` 指令，否者可能导致复用。
* `v-on:监听 DOM 事件` 简写 `@`，事件绑定的方法，例如 `v-on:click="fn"`，如果 `fn` 中不传递参数可以不写`()` `Vue` 会自动传入事件源 `event` 的属性，如果 `fn` 需要传参就加上`($event, parm)`才可以拿到事件源。`v-on:` 也可以监听自定义的事件，常用于父子组件的传递。
...
### Vue 中的 Watch 观察者属性
> Watch 是一个对象，用来观察 `data` 中某个值的变化执行对应的函数，函数接收两个参数`newValue, oldValue` 代表变化后的值和变化前的值。
